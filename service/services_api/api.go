...
func Get_Bing_Picture_URL() (url_img string, name_img string) {
	...
	if err != nil {
		log.Infoln("请求[https://cn.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1&mkt=zh-CN]错误")
		log.Infoln(err)
		return "", ""
	}
...
func Get_Random_Picture_URL() (url_img string, name_img string) {
	...
	if err != nil {
		log.Infoln("请求[https://api.ixiaowai.cn/api/api.php?return=json]错误")
		log.Infoln(err)
		return "", ""
	}
	defer resp.Body.Close()
	...
	if err != nil {
		log.Infoln("解析[https://api.ixiaowai.cn/api/api.php?return=json]请求内容错误")
		log.Infoln(err)
		return "", ""
	}
